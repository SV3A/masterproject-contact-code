...
% 'animate' animates the numerical solution.
  ...
  imp_field = uicontrol('style', 'text', ...
                        'string', ['0/', num2str(ceil(length(te_total)/2))], ...
                        'backgroundcolor', [1 1 1], ...
                        'fontname', 'Monospaced', ...
                        'fontsize', 14, ...
                        'horizontalalignment', 'left', ...
  ...
  sld_handle = uicontrol('style','slider',...
                'backgroundcolor', [1 1 1], ...
                'units','normalized',...
                'position',[0.01 0.04 0.4 0.01],...
                'min',1,'max',length(t_total),'val',1,...
                'SliderStep', [30/length(t_total) 30/length(t_total)]);

  % Create eventlistener for a change of slider value
  addlistener(sld_handle,'Value','PostSet', @(~,~) updatefig(sld_handle, s, ...
              y_total, t_total, te_total, pos1, rotor, stator, rotor_cross, ...
              stator_cross, rev_mark, force_plt, forcein_plt, fplt, F_r, ...
              v_vector, v_rel_r, F_cxs, F_cys, deltas, delta_ds, ...
              delta_d_ini, s_total, info_field, imp_field));

  % Add slider sensitivity control
  th = uicontrol('style','togglebutton','string','Fine Step','fontsize', 12, ...
                'units','normalized', ...
                'position',[0.44 0.025 0.06 0.0375]);

  % Eventlistener for sens control
  addlistener(th, 'Value', 'PostSet', ...
    @(~,~) toggleSldSens(th.Value, sld_handle, t_total));

  % Add buttons the jump forward and backward in impacts
  fwd_button = uicontrol('style', 'pushbutton', 'string', 'Next', ...
                         'fontsize', 12, ...
                         'units', 'normalized', ...
                         'position',[0.06 0.10 0.04 0.0375]);

  bwd_button = uicontrol('style', 'pushbutton', 'string', 'Previous', ...
                         'fontsize', 12, ...
                         'units', 'normalized', ...
                         'position',[0.015 0.10 0.04 0.0375]);

  % Eventlisteners
  set(fwd_button, 'Callback', @(~, ~) stepImpact(fwd_button.String, ...
    sld_handle, t_total, te_total));
  set(bwd_button, 'Callback', @(~, ~) stepImpact(bwd_button.String, ...
    sld_handle, t_total, te_total));

  % Add listener for keyboard shortcuts
  set(mainfig, 'KeyPressFcn', @(src, event) keypress(src, event, th, ...
    sld_handle, t_total, te_total));

  set(sld_handle, 'KeyPressFcn', @(src, event) keypress(src, event, th, ...
    sld_handle, t_total, te_total));
